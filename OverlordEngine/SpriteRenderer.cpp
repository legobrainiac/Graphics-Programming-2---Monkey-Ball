...
void SpriteRenderer::UpdateBuffer(const GameContext& gameContext)
{
	...
	if (!m_pVertexBuffer || m_Sprites.size() > m_BufferSize)
	{
		...
		desc.ByteWidth = sizeof(SpriteVertex) * m_BufferSize;
		desc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
		desc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
		desc.MiscFlags = 0;

		const auto hr = gameContext.pDevice->CreateBuffer(&desc, nullptr, &m_pVertexBuffer);
		if (Logger::LogHResult(hr, L"SpriteRenderer::UpdateBuffer > Vertexbuffer creation failed!"))
			return;
	}
	...
	memcpy(mappedResource.pData, &m_Sprites[0], sizeof(SpriteVertex) * m_BufferSize);
